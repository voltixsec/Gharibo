 ...
 * WHAT THIS PRODUCES
 * ------------------
 * `scripts/eval/kaggle/gharibo-eval-002.ipynb`, the notebook that runs the ONE authorized
 * held-out TEST benchmark (attempt #6, DEC-0046) on a free Kaggle T4: BASE then CANDIDATE over the
 * same 80 TEST prompts, greedy, identical settings for both arms.
 *
 * DEC-0046 authorizes exactly ONE kernel push. The pins carry that bound so the notebook asserts it
 * at runtime, and the prior authorization (DEC-0044) is carried alongside rather than overwritten —
 * this attempt supersedes DEC-0044 for execution without erasing it.
 *
...
const OUT_REL = "scripts/eval/kaggle/gharibo-eval-002.ipynb";
...
export const EVAL_PINS = Object.freeze({
  ...
  /**
   * The exact distribution repo for the local-snapshot download (DEC-0042 architecture).
   * The loader model id is `unsloth/gpt-oss-20b` (what Unsloth resolves internally), but the
   * snapshot_download call targets the bnb-4bit distribution repo explicitly.
   */
  distributionRepo: "unsloth/gpt-oss-20b-unsloth-bnb-4bit",
  ...
  authorizationDecisionId: "DEC-0046",
   ...
   */
  priorAuthorizationDecisionId: "DEC-0044",
  attemptNumber: 6,
...
export function pinsCell(p) {
  ...
  return code([
    ...
    "",
    "assert PINS['authorizationDecisionId'] == 'DEC-0046', 'unexpected authorization'",
    "assert PINS['decision'] == 'AUTHORIZED WITH LIMITS', 'unexpected decision scope'",
    "assert PINS['attemptNumber'] == 6, 'unexpected attempt number'",
    "assert PINS['maximumKernelPushes'] == 1, 'attempt #6 is bounded to ONE kernel push'",
    "assert PINS['priorAuthorizationDecisionId'] == 'DEC-0044', 'unexpected prior authorization'",
    "",
  ...
  return [
    {
      ...
      metadata: {},
      source: md([
        "# GHARIBO — governed held-out TEST evaluation (attempt #6, `DEC-0044`)",
        "",
        "| Field | Value |",
        "|-------|-------|",
    ...
    },
    {
      ...
      outputs: [],
      source: code([
        "# --- LOCAL-SNAPSHOT DOWNLOAD (DEC-0042 architecture) ---",
        "#",
        "# The primary invariant: THE LOADER RECEIVES A LOCAL DIRECTORY, NOT A REPO ID.",
        "#",
        "# DEC-0041 proved that snapshot_download at the immutable SHA caches files but does NOT",
        "# prevent the loader from making live API calls to 'main' for metadata lookups",
        "# (tree/main/additional_chat_templates — 404 at 'main', fatal).",
        "#",
        "# DEC-0042 closes that gap: CAPTURE the returned local snapshot path and pass it",
        "# directly to the loader with local_files_only=True.",
        "#",
        "# HF_HUB_DISABLE_XET=1 is preventive hardening (recorded, not assumed causal).",
        "os.environ['HF_HUB_DISABLE_XET'] = '1'",
        "print('HF_HUB_DISABLE_XET=1 (preventive hardening)')",
        "",
        "# --- post-install Hugging Face Hub refresh barrier ---",
        "# Attempt #6 v4 proved that importing huggingface_hub before the governed uv install",
        "# and then importing more of it after the install can leave one Python process with",
        "# mixed in-memory module state. The observed failure was snapshot_download code",
        "# referencing a constant absent from the already-loaded constants module.",
        "#",
        "# The package install has completed at this point. Purge ONLY huggingface_hub modules",
        "# from sys.modules, invalidate import caches, then import one coherent on-disk version.",
        "_HFHUB_PRELOADED = sorted(",
        "    name for name in sys.modules",
        "    if name == 'huggingface_hub' or name.startswith('huggingface_hub.')",
        ")",
        "print('huggingface_hub modules before post-install refresh:', len(_HFHUB_PRELOADED))",
        "",
        "for _name in sorted(_HFHUB_PRELOADED, key=len, reverse=True):",
        "    del sys.modules[_name]",
        "",
        "import importlib as _importlib",
        "_importlib.invalidate_caches()",
        "import importlib.metadata as _hf_metadata",
        "",
        "import huggingface_hub as _hfhub",
        "from huggingface_hub import snapshot_download",
        "",
        "_hf_dist_version = _hf_metadata.version('huggingface-hub')",
        "print('huggingface_hub module version:', _hfhub.__version__)",
        "print('huggingface_hub dist version  :', _hf_dist_version)",
        "print('huggingface_hub module path   :', _hfhub.__file__)",
        "assert _hfhub.__version__ == _hf_dist_version, (",
        "    f'REFUSING TO RUN: huggingface_hub module/distribution skew: '",
        "    f'{_hfhub.__version__!r} != {_hf_dist_version!r}'",
        ")",
        "print('HF_HUB_POST_INSTALL_REFRESH_PASS')",
        "",
        ...
        "",
        "assert SNAPSHOT_DIR is not None, 'REFUSING TO RUN: snapshot_download returned None'",
        "assert os.path.isdir(SNAPSHOT_DIR), f'REFUSING TO RUN: snapshot_dir is not a directory: {SNAPSHOT_DIR}'",
        "_snapshot_basename = os.path.basename(SNAPSHOT_DIR.rstrip('/'))",
        "assert _snapshot_basename == _dist_rev, (",
        "    f'REFUSING TO RUN: snapshot basename {_snapshot_basename!r} != immutable SHA {_dist_rev!r}'",
        ")",
        "assert _snapshot_basename != 'main', 'REFUSING TO RUN: snapshot resolved to mutable ref main'",
        "print(f'snapshot verified: {_snapshot_basename}')",
      ]),
    },
    {
      cell_type: "code",
      ...
      outputs: [],
      source: code([
        "# --- SEAL NETWORK RESOLUTION (DEC-0042 architecture) ---",
        "#",
        "# After the complete snapshot exists, the MODEL-LOAD phase must NOT depend on",
        "# Hugging Face Hub resolution. HF_HUB_OFFLINE=1 and TRANSFORMERS_OFFLINE=1 are",
        "# defense-in-depth. The PRIMARY invariant is the local directory passed to the loader.",
        "os.environ['HF_HUB_OFFLINE'] = '1'",
        "os.environ['TRANSFORMERS_OFFLINE'] = '1'",
        "print('HF_HUB_OFFLINE=1 (defense-in-depth)')",
        "print('TRANSFORMERS_OFFLINE=1 (defense-in-depth)')",
        "print(f'loader will receive local snapshot: {SNAPSHOT_DIR}')",
      ]),
    },
    {
      cell_type: "code",
      ...
      outputs: [],
      source: code([
        "# --- load BASE, unadapted (LOCAL-SNAPSHOT architecture, DEC-0042) ---",
        "#",
        ...
        "# resolves internally, and no revision for the underlying base repo exists on it.",
        "#",
        "# DEC-0040/DEC-0041 failure: the loader was called with the repo id, which allowed",
        "# Hub resolution to hit tree/main/additional_chat_templates (404 at 'main', fatal).",
        "#",
        "# DEC-0042 fix: the loader receives the LOCAL SNAPSHOT DIRECTORY (captured above),",
        "# not a repo id. local_files_only=True prevents any network resolution.",
        "# The primary invariant: THE LOADER RECEIVES A LOCAL DIRECTORY, NOT A REPO ID.",
        "#",
        "# The pinned base revision is asserted against the LIVE revision of the",
        "# real base repo (`openai/gpt-oss-20b`) in the identity cell above.",
        ...
        "base_model.eval()",
        "",
        "# The loader must not have silently substituted a different repo.",

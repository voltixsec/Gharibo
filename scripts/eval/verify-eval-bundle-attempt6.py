#!/usr/bin/env python
"""
verify-eval-bundle-attempt6.py — deterministic PASS/FAIL privacy + integrity verifier for the governed
Kaggle evaluation launch bundle.

WHY THIS EXISTS
---------------
`prepare-eval-launch-attempt6.mjs` proves the bundle was BUILT correctly. This script proves the bundle
...
Usage:
  python scripts/eval/verify-eval-bundle-attempt6.py            # report
  python scripts/eval/verify-eval-bundle-attempt6.py --quiet    # exit code only
"""
...
def main() -> int:
    ...
    if not BUNDLE.exists():
        ...
        print("  run: node scripts/eval/prepare-eval-launch-attempt6.mjs")
        return 1
    ...
    if plan_path.exists():
        ...
        nb = BUNDLE / "kernel/gharibo-eval-002.ipynb"
        if nb.exists():
            check(
        ...
        check("launch-plan.json present", False, "missing — run prepare-eval-launch-attempt6.mjs", quiet)

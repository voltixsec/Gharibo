 ...
 * WHAT THIS GUARDS
 * ----------------
 * `scripts/eval/kaggle/gharibo-eval-002.ipynb` is the ONLY thing permitted to run the single
 * authorized held-out TEST benchmark (attempt #6, DEC-0046). This checker re-reads the GENERATED
 ...
 *   6. AUTHORIZATION— the DEC-0046 pins are present, the decision string is exact, and the
...
check("authorization decision id is DEC-0046", EVAL_PINS.authorizationDecisionId === "DEC-0046");
check(
...
check("the run is bounded to attempt #6", EVAL_PINS.attemptNumber === 6);
check(
  "attempt #6 permits exactly ONE kernel push",
  EVAL_PINS.maximumKernelPushes === 1,
  `found ${JSON.stringify(EVAL_PINS.maximumKernelPushes)} — an unbounded attempt is not authorized`
);
check(
  "the prior authorization is recorded as DEC-0044",
  EVAL_PINS.priorAuthorizationDecisionId === "DEC-0044",
...
check(
  ...
  /assert\s+PINS\['authorizationDecisionId'\]\s*==\s*'DEC-0046'/.test(codeSource),
  "a runtime authorization assertion must guard the run"
);
check(
  "the notebook runtime guard asserts attempt #6",
  /assert\s+PINS\['attemptNumber'\]\s*==\s*6/.test(codeSource),
  "the generated notebook must refuse to run under any other attempt number"
);
check(
  "the notebook runtime guard records DEC-0044 as the prior authorization",
  /assert\s+PINS\['priorAuthorizationDecisionId'\]\s*==\s*'DEC-0044'/.test(codeSource),
  "Attempt #6 must preserve the immediately preceding authorization identity"
);
check(
  "the notebook asserts the ONE-push bound before doing any work",
  /assert\s+PINS\['maximumKernelPushes'\]\s*==\s*1/.test(codeSource),
  "the run must refuse to start if the attempt bound has been loosened"
);
